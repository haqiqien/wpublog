<x-layout :title="$title">
    ...
    <div class="py-4 px-4 mx-auto max-w-screen-xl  lg:px-6">
        ...
        {{ $posts->links() }}
        <div class="mt-4 mb-4 grid gap-8 lg:grid-cols-3 md:grid-cols-2">
            @forelse ($posts as $post)
            <article class="p-6 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                <div class="flex justify-between items-center mb-5 text-gray-500">
                    <a href="/posts?category={{ $post->category->slug }}" class="text-gray-900  hover:underline"><span class="{{ $post->category->color }} text-grey-800 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded dark:bg-primary-200 dark:text-primary-800">{{ $post->category->name }}</span></a>
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <span class="font-medium text-xs dark:text-white">{{ $post->author->name }}</span>
                    </div>
                    </a>
